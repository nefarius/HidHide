...
class Build : NukeBuild
{
    ...
    static void SignWithPfx(string pfxPath, string password, AbsolutePath file, bool pageHash, bool timestamp)
    {
    ...
    /// <summary>Runs <c>wdkwhere run signtool …</c> (Nefarius.Tools.WDKWhere) so the kit signtool path is resolved without hard-coding kit versions.</summary>
    static void RunWdkWhereSigntool(params string[] signtoolArguments)
    {
        var escaped = string.Join(" ", signtoolArguments.Select(EscapeForCmdToken));
        ProcessTasks.StartProcess(
                "wdkwhere",
                $"run signtool {escaped}",
                logInvocation: false,
                logger: (_, s) => Logger.Normal(s))
            .AssertZeroExitCode();
    }
